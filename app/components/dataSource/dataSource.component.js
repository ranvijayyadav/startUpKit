"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require("@angular/core");
var DataSourceComponent = (function () {
    function DataSourceComponent() {
        this.isQuerySelected = false;
        this.isMicroServiceSelected = false;
    }
    DataSourceComponent.prototype.microServiceHandler = function () {
        this.typeSelection(false, true);
        //this.isQuerySelected=false;
        //this.isMicroServiceSelected=true;
    };
    DataSourceComponent.prototype.queryHandler = function () {
        this.typeSelection(true, false);
        //this.isMicroServiceSelected=false;
        //this.isQuerySelected=true;
    };
    DataSourceComponent.prototype.cancelHandler = function () {
        this.typeSelection(false, false);
    };
    DataSourceComponent.prototype.validateHandler = function () {
    };
    DataSourceComponent.prototype.saveHandler = function () {
        this.typeSelection(false, false);
    };
    DataSourceComponent.prototype.typeSelection = function (isQuery, isMicro) {
        this.isMicroServiceSelected = isMicro;
        this.isQuerySelected = isQuery;
    };
    return DataSourceComponent;
}());
DataSourceComponent = __decorate([
    core_1.Component({
        selector: 'data-source',
        templateUrl: "./app/components/dataSource/dataSource.component.html"
    }),
    __metadata("design:paramtypes", [])
], DataSourceComponent);
exports.DataSourceComponent = DataSourceComponent;
//# sourceMappingURL=dataSource.component.js.map